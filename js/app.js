...
function addNewSection() {
	...
	// and before that other element acting as last child in the main, always that after that element there is not a section.
	...
	if (main.hasChildNodes) {
		...
		for (let mainNode of main.children) {
	...
	// If we have 'green light' then we proceed to create and append the new section
...
function setViewportActivate() {
	...
	for (const section of sections) {
		...
		// Removes sections as active and navigation as highlighted
		if (sectionDomRect.top > 130) {
			console.log('top out of view');
			sections[i].classList.remove('your-active-class');
			let navLink = document.querySelectorAll('li');
			navLink[i].classList.remove('nav-highlighted');
		}

		// Set sections as active and navigation gets highlighted
		if (sectionDomRect.top < 129) {
			console.log(sections[i]);
			sections[i].classList.add('your-active-class');
			let navLink = document.querySelectorAll('li');
			navLink[i].classList.add('nav-highlighted');
		}

		// Removes sections as active and navigation as highlighted
		if (sectionDomRect.bottom < 128) {
			console.log('top out of view');
			sections[i].classList.remove('your-active-class');
			let navLink = document.querySelectorAll('li');
			navLink[i].classList.remove('nav-highlighted');
    ...
    // This will help that scrolling don't try to add or remove the your-active-class to an element tha doesn't exist.
		i++;
